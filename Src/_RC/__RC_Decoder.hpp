...
class RcDecoder /*: public ISubject*/{
  public:
          void      putSample (void);                                 //  Положить отсчет времени пришедшего импульса
  inline  uint16_t  getMessage (void)  { return _message; }           //  Возвращает текущую принятую команду
  inline  void      timerHandler (uint16_t time)  { if(_timer < 60000) { _timer += time; } else { _inputMessage =0; _message =0; } } //  Обработчик таймера. T>20млс - окончание передачи
...
//  void  notyfyObservers (void);
  ...
  private:
    uint16_t _timer;              //  Таймер, считает интервал между импульсами [мкс]
