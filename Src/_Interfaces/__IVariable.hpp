...
class IVariable{
  public:
    ...
    virtual inline uint32_t getValue  (void) =0;            // Возвращает значение параметра
    virtual inline void     setValue  (uint32_t value) =0;  // Устанавливает значение параметра
    // Методы, возвращающие атрибуты
    virtual inline uint32_t getMin    (void) =0;            // Возвращает минимальное значение параметра
    virtual inline uint32_t getMax    (void) =0;            // Возвращает максимальное значение параметра
    ...
    virtual inline uint32_t getEditingValue(void) =0;       // Выход из редактирования параметра (без сохранения результата)
};  
